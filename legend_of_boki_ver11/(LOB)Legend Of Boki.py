...
class WindowClass(QMainWindow, game):
    ...
    def __init__(self):
        ...
        self.portal_sample.setScaledContents(True)
        self.movie.start()
        ...
        self.Portion_11_Btn.clicked.connect(self.tent_use_using)
    ...
    def skillbox(self):
        ...
        self.skill_6 = SkillOption("맵 핵", None, 0, 0, 1, 1, 1000)
        ...
        self.skill_11 = SkillOption("집중타", None, random.uniform(1.2, 1.5), 0.1, 1, 1, 1)
    ...
    def go_to_normalfield(self):
        ...
        self.StackWidget_Item.setEnabled(True)
    ...
    def btnname(self, obnum, classatk, classmp, classnum):
        ...
        self.skill_button_number = obnum
    ...
    def skill_heal(self, num, classnum):
        ...
        if self.choice_btn == '힐':
            ...
            if self.class_mp_dict[classnum] < 10:
            ...
            if self.class_mp_dict[classnum] < 20:
                self.Log_textEdit.append('마나가 부족합니다')
    ...
    def Iteam_function(self, num):
        """아이템 선택시 나머지 버튼 잠김"""
        self.cocont = 0
        for i in self.item_list[0:11]:
            self.cocont += i.stack
        if self.cocont == 0:
            self.show_messagebox("가방을 뒤적뒤적했더니.. 아무것도없었는데..\n시간만 버렸어요.. 턴이 넘어가요!!")
            if self.user_turn >= 5:
                self.user_turn = 0
                self.monster_attack_users()
                return
        ...
        if self.attackType == 1:
            if self.target == 2:
                ...
                if self.skill_button_number == 8 or self.skill_button_number == 15:
                    ...
                    self.ice_effect_all(self.all_number)
    ...
    def hp_using_1(self):
        ...
        self.Class_6_Btn.clicked.connect(self.item_last_pass)
    ...
    def revive_using(self):
        ...
        self.class_6_Btn.clicked.connect(self.revive_rest)
    ...
    def revive_rest(self):
        self.class_hp_dict_last[0] = self.list_class[-1][0].get_maxhp()
        self.class_mp_dict_last[0] = self.list_class[-1][0].get_maxmp()
        self.Log_textEdit.append("체력과 마나를 전부 회복했습니다")
    ...
    def item_last_pass(self):
        self.item_list[0].stack -= 1
        self.StackWidget_Item.setCurrentWidget(self.Page_Use)
        self.Log_textEdit.append("그는 출전하지않아서 먹지 못해요 애꿏은 포션만 땅에 버렸네요..")
        for i in range(1, 7):
            getattr(self, f'Class_{i}_Btn').disconnect()
        return

    def item_set(self):
        self.item_consumable_hp_S = Consumable("HP포션 (소)", 50, 0)
        self.item_consumable_hp_M = Consumable("HP포션 (중)", 100, 0)
        self.item_consumable_hp_L = Consumable("HP포션 (대)", 150, 0)
        self.item_consumable_mp_S = Consumable("MP포션 (소)", 50, 0)
    ...
    def fire_effect(self, num):
        self.effect_widget_list[num].show()
        self.effect_widget_list[num].setScaledContents(True)
        self.effect_widget_list[num].setAlignment(Qt.AlignCenter)
        self.effect_widget_list[num].setMovie(self.attack_effect_fire)
        self.attack_effect_timer.start(1800)
        self.attack_effect_fire.start()
    ...
    def fire_effect_all(self, num):
        for i in range(0, num-1):
            ...
            self.effect_widget_list[i].setMovie(self.attack_effect_fire)
        self.attack_effect_timer.start(1800)
        self.attack_effect_fire.start()
        self.attack_effect_timer.timeout.connect(self.effect_close)
    ...
    def ice_effect_all(self, num):
        for i in range(0, num-1):
            ...
            self.effect_widget_list[i].setScaledContents(True)
            self.effect_widget_list[i].setAlignment(Qt.AlignCenter)
            self.effect_widget_list[i].setMovie(self.attack_effect_ice)
        self.attack_effect_timer.start(1000)
        self.attack_effect_ice.start()
        self.attack_effect_timer.timeout.connect(self.effect_close)
    ...
    def thunder_effect_all(self, num):
        for i in range(0, num-1):
            ...
            self.effect_widget_list[i].setScaledContents(True)
            self.effect_widget_list[i].setAlignment(Qt.AlignCenter)
            self.effect_widget_list[i].setMovie(self.attack_effect_thunder)
        self.attack_effect_timer.start(1000)
        self.attack_effect_thunder.start()
        self.attack_effect_timer.timeout.connect(self.effect_close)

    def bow_effect(self, num):
        self.effect_widget_list[num].show()
        self.effect_widget_list[num].setScaledContents(True)
        self.effect_widget_list[num].setAlignment(Qt.AlignCenter)
        self.effect_widget_list[num].setMovie(self.attack_effect_bow)
        self.attack_effect_timer.start(1000)
        self.attack_effect_bow.start()
        self.attack_effect_timer.timeout.connect(self.effect_close)
    ...
    def effect_close(self):
        self.attack_effect_timer.stop()
        for i in range(len(self.effect_list)):
            self.effect_list[i].stop()
        for j in range(len(self.effect_widget_list)):
            self.effect_widget_list[j].close()
