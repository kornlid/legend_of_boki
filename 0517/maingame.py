...
class WindowClass(QMainWindow, main_game):
    def __init__(self):
        ...
        list_class = [class_1, class_2, class_3, class_4, class_5, class_6]  # 클래스들 리스트에 담기
        random.shuffle(list_class)  # 리스트에 담은 캐릭터 랜덤으로 섞기
        ...
        class_images = [self.warrior, self.white_wizard, self.black_wizard, self.red_wizard, self.archer,self.sword_man] #이미지 리스트화
    ...
    # 함수로 키 입력 단순화 해보려고 했으나 실패. 시도했지만 코드를 줄이는데 큰 영향을 안 줌. (기록용으로 주석처리 / 삭제예정)
    ...
    def battle_ground(self):
        ...
        for btn in range(5):
            ...
            getattr(self, f'Status{btn + 1}_Action2_Skill').clicked.connect(
                lambda state, index_=btn + 1: self.skill_btn(index_))
