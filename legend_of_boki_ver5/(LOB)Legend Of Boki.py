...
class WindowClass(QMainWindow, game):
    ...
    def __init__(self):
        ...
        # 유저 턴 구분 초기 설정값
        self.user_turn = 0
        self.class_turn = [0,0,0,0,0]
        self.class_list = [] #팝된 유저 값 넣을 공간
        self.class_name = [] #팝된 유저 이름값 넣을 공간
        self.classhp_list = [] #팝된 유저 체력값 넣을 공간
        self.classmaxhp_list = [] #팝된 유저 맥스 체력값 넣을 공간
        self.StautsHplist = [] #팝된 체력 리스트 넣을 공간
        # ui 하단 버튼 비활성화
        for i in range(1, 6):
        ...
        self.class_name_list = [self.Status1_1_Name,self.Status2_1_Name,self.Status3_1_Name,self.Status4_1_Name,self.Status5_1_Name]
        self.class_hp_list = [self.Status1_2_HpValue,self.Status2_2_HpValue,self.Status3_2_HpValue,self.Status4_2_HpValue,self.Status5_2_HpValue]
        # 혜빈 코드 취합 본 끝 ============================================================================================
    ...
    def battleclear(self):
        """MonsterImage 전부 처치 이후 버튼 클리어 함수"""
    ...
    # 유저 턴 넘기는 함수
    def user_Turn(self):  # MonsterImage 공격 버튼 눌렀을 때 함수
        for i in range(1, 6):
        ...
        for i in self.actbtnbox:
            i.setEnabled(False)

        for idx, turn in enumerate(self.frame_class_list):
            if self.user_turn == idx:
                self.frame_class_list[self.user_turn].setEnabled(False)
                self.frame_class_list[self.user_turn-len(self.frame_class_list)+1].setEnabled(True)

        if len(self.frame_class_list) == 1:
            self.Monster_Turn()
            return
        ...
        print(len(self.frame_class_list))

        if self.user_turn >= len(self.frame_class_list):
            self.frame_class_list[-1].setEnabled(False)
            self.user_turn = 0
            self.Monster_Turn()
            return
    ...
    def Monster_Turn(self):
        self.num = 0
        self.guardoption()
        self.monlifecnt = 0  # 존재하는 MonsterImage 마리수
            ...
            if getattr(self, f'Monster_{num}_Name').text() != "":
                for monster in self.monsterbox:
                    if monster.name == getattr(self, f'Monster_{num}_Name').text():
                        self.classnum = random.randrange(0,len(self.StautsHpall))
                        if self.StautsHpall[self.classnum] > monster.atk:
                            self.StautsHpall[self.classnum] = self.StautsHpall[self.classnum]-monster.atk
                            self.class_hp_list[self.classnum].setText(str(self.StautsHpall[self.classnum]) + "/" + str(self.Statusclass[self.classnum].get_maxhp()))
                            self.Log_textEdit.append("%s이 %s에게 데미지를 %d만큼 입혔습니다."%(monster.name,self.class_name_list[self.classnum].text(),monster.atk))
                        else:
                            self.StautsHpall[self.classnum] = 0
        for idx, hp in enumerate(self.StautsHpall):
            if hp == 0:
                self.Log_textEdit.append("%s가 사망하였습니다." % self.class_name_list[idx].text())
                self.class_hp_list[idx].setText(
                    str(self.StautsHpall[idx]) + "/" + str(self.Statusclass[idx].get_maxhp()))
                self.class_name.append(self.class_name_list.pop(idx)) # 클래스 이름값 뺌
                self.class_list.append(self.frame_class_list.pop(idx)) # 클래스 자체를 뺌
                self.classhp_list.append(self.class_hp_list.pop(idx)) # 클래스 현재 체력값 뺌
                self.classmaxhp_list.append(self.Statusclass.pop(idx)) # 클래스 맥스 체력값 뺌
                self.StautsHplist.append(self.StautsHpall.pop(idx))
        if self.StautsHplist.count(0) == 5:
            self.StackWidget_Field.setCurrentIndex(0)
        # find1234
    ...
    def Show_Dungeon_Entrance(self, map_num):
        ...
        self.boss_monster.move(random.randint(self.map_size[map_num][0], self.map_size[map_num][1]),
                               random.randint(self.map_size[map_num][2], self.map_size[map_num][3]))  # 보스 MonsterImage 랜덤으로 등장
    ...
    def nomalact(self, num):
        self.attackType = 0
        self.choice_btn = num * 10 # 딜이 10배 증가시켜놓으려고
        for actbtn in self.actbtnbox:
        ...
        if get == '1':
            ...
            for i in range(1,6):
                if i == num:
                    ...
                    getattr(self, f'Status{num}_Action2_Skill').setDisabled(True)
                    getattr(self, f'Status{num}_Action3_Item').setDisabled(True)
                    getattr(self, f'Status{num}_Action4_Run').setDisabled(True)
                else:
                    getattr(self, f'Status{i}_Action1_Attack').setDisabled(True)
                    getattr(self, f'Status{i}_Action2_Skill').setDisabled(True)
                    getattr(self, f'Status{i}_Action3_Item').setDisabled(True)
                    getattr(self, f'Status{i}_Action4_Run').setDisabled(True)
        elif get == '2':
            self.Log_textEdit.append("스킬 공격을 선택하였습니다.")
            for i in range(1, 6):
                if i == num:
                    getattr(self, f'Status{num}_Action1_Attack').setDisabled(True)
