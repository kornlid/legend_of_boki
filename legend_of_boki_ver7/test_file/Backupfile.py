...
class WindowClass(QMainWindow, game):
    ...
    def Add_Monster_info(self):
        ...
        for num in range(1, monster_random_num):
            getattr(self, f'Monster_{num}_Name').setText(getattr(self, f'nomalfield_fire_monster{1}').name)  # MonsterImage 이름
            getattr(self, f'Monster_{num}_Name').setStyleSheet("Color : white")
            getattr(self, f'Monster_{num}_QLabel').setPixmap(
                QPixmap(getattr(self, f'nomalfield_fire_monster{1}').image))  # MonsterImage 이미지
            getattr(self, f'Monster_{num}_QProgressBar').setMaximum(
                getattr(self, f'nomalfield_fire_monster{1}').hp)  # MonsterImage 체력
            self.monster_hp_dict[num] = getattr(self, f'nomalfield_fire_monster{1}').hp  # MonsterImage 체력 더하기
    ...
    def monster_got_damage(self, num):
        ...
        if self.attackType == 1:
            ...
            self.monster_hp_dict[num] -= self.choice_btn  # 임시로 MonsterImage 체력에는 100씩 데미지 입힘
